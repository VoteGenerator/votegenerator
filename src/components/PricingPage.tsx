...
import { Check, X, Zap, Crown, Users, BarChart3, Clock, ArrowRight, Star, ChevronDown, Sparkles, Shield, BadgeCheck, CreditCard, Loader2, Share2, Palette, CheckSquare, Globe } from 'lucide-react';
...
function PricingPage(): React.ReactElement {
    ...
    const TIERS = [
        { id: 'free', name: 'Free', tagline: 'No credit card required', price: 0, icon: Users, color: 'slate', cta: 'Create Free Poll', ctaLink: '/create', badge: 'Forever Free', features: { responses: '50 responses', duration: '7 days active', polls: 'Unlimited free polls', highlights: ['6 free poll types', 'QR code sharing', 'Real-time results', 'Embed on your site'] }},
        { id: 'starter', name: 'Starter', tagline: 'For your next event', price: prices.starter, icon: Zap, color: 'blue', cta: 'Get Starter', ctaLink: '/.netlify/functions/vg-checkout?tier=starter', badge: 'One-Time Payment', features: { responses: '500 responses', duration: '30 days active', polls: '1 premium poll', highlights: ['Everything in Free', 'Export to CSV', 'Device breakdown stats', '90-day data retention'] }},
        { id: 'pro_event', name: 'Pro Event', tagline: 'For important events', price: prices.proEvent, popular: true, icon: Crown, color: 'purple', cta: 'Get Pro Event', ctaLink: '/.netlify/functions/vg-checkout?tier=pro_event', badge: 'One-Time Payment', features: { responses: '2,000 responses', duration: '60 days active', polls: '1 premium poll', highlights: ['Everything in Starter', 'Visual Poll (images)', 'Export PDF & PNG', 'Remove VG branding', 'Custom short link'] }},
        { id: 'unlimited', name: 'Unlimited', tagline: 'For power users', price: prices.unlimited, periodNote: '1 year access', icon: Star, color: 'amber', cta: 'Get Unlimited', ctaLink: '/.netlify/functions/vg-checkout?tier=unlimited', badge: 'Best Value', features: { responses: '5,000 per poll', duration: '1 year per poll', polls: 'Unlimited premium polls', highlights: ['Everything in Pro Event', 'Unlimited premium polls', 'Upload your logo', 'Priority support'] }},
    ];
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
            ...
            <div className="max-w-7xl mx-auto px-4 pt-16 pb-12 text-center">
                ...
                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-xl text-slate-600 max-w-2xl mx-auto mb-6">Start free. Pay only when you need more.</motion.p>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="inline-flex items-center gap-3 px-6 py-3 bg-emerald-50 border border-emerald-200 rounded-xl mb-4">
                    ...
                    <div className="text-left"><p className="text-emerald-900 font-bold">All Paid Plans Are One-Time Payments</p><p className="text-emerald-700 text-sm">No subscriptions. No recurring charges.</p></div>
                </motion.div>
                    ...
                    {TIERS.map((tier, index) => {
                        ...
                        return (
                            <motion.div key={tier.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + index * 0.05 }}
                                ...
                                {tier.popular && <div className="absolute top-0 left-0 right-0 bg-purple-600 text-white text-center py-1.5 text-sm font-medium"><Star className="inline-block mr-1" size={14} /> Most Popular</div>}
                                <div className={`p-6 flex-1 flex flex-col ${tier.popular ? 'pt-12' : ''}`}>
                                    <span className={`self-start px-2.5 py-1 rounded-full text-xs font-bold mb-3 ${tier.id === 'unlimited' ? 'bg-amber-100 text-amber-700' : tier.id === 'free' ? 'bg-slate-100 text-slate-600' : 'bg-blue-100 text-blue-700'}`}><CreditCard size={10} className="inline mr-1" />{tier.badge}</span>
                                    <div className={`w-12 h-12 ${colors.bg} rounded-xl flex items-center justify-center mb-4`}><Icon className={colors.text} size={24} /></div>
                                    <h3 className="text-xl font-bold text-slate-900">{tier.name}</h3>
                                    <p className="text-slate-500 text-sm mt-1">{tier.tagline}</p>
                                    <div className="mt-4 mb-2">
                                        {tier.price === 0 ? <span className="text-4xl font-bold text-slate-900">Free</span> : loading ? <span className="text-4xl font-bold text-slate-300">...</span> : <><span className="text-4xl font-bold text-slate-900">{prices.symbol}{tier.price}</span><span className="text-slate-500 text-sm ml-1">{currency}</span></>}
                                    </div>
                                    ...
                                    <ul className="space-y-2 mb-6 flex-1">{tier.features.highlights.map((feature, i) => <li key={i} className="flex items-start gap-2 text-sm text-slate-600"><Check size={16} className="text-emerald-500 flex-shrink-0 mt-0.5" />{feature}</li>)}</ul>
                                    <a href={tier.ctaLink} className={`w-full py-3 ${colors.button} font-medium rounded-xl transition flex items-center justify-center gap-2`}>{tier.cta} <ArrowRight size={18} /></a>
                                </div>
                            </motion.div>
                        );
                    })}
            ...
            <div className="max-w-4xl mx-auto px-4 pb-16">
                ...
                <div className="space-y-4">
                    {[
                        ...
                        { q: 'What currency will I be charged in?', a: `Prices are shown in ${currency}. Stripe handles the checkout securely.` },
