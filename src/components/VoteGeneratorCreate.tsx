...
const POLL_TYPES = [
    { id: 'multiple', name: 'Multiple Choice', icon: CheckSquare, shortDesc: 'Pick one or more options', tooltip: 'The classic poll type. Voters click their favorite(s). Perfect for quick team decisions like lunch votes or meeting topics.', gradient: 'from-blue-500 to-indigo-500', bgGlow: 'bg-blue-500/20', tier: 'free' as PollTier },
    { id: 'ranked', name: 'Ranked Choice', icon: ListOrdered, shortDesc: 'Drag to rank in order', tooltip: 'Voters rank all options from favorite to least favorite. Shows true preferences and avoids vote splitting. Used in elections!', gradient: 'from-indigo-500 to-purple-500', bgGlow: 'bg-indigo-500/20', tier: 'free' as PollTier },
    { id: 'pairwise', name: 'This or That', icon: ArrowLeftRight, shortDesc: 'Quick A vs B comparisons', tooltip: 'Shows two options at a time. Voters pick their favorite. Great for narrowing down many options with gut-reaction feedback.', gradient: 'from-orange-500 to-red-500', bgGlow: 'bg-orange-500/20', tier: 'free' as PollTier },
    { id: 'meeting', name: 'Meeting Poll', icon: Calendar, shortDesc: 'Find the best time', tooltip: 'Like Doodle but simpler! Add date/time options and everyone marks when they\'re available. No more email chains.', gradient: 'from-amber-500 to-orange-500', bgGlow: 'bg-amber-500/20', tier: 'free' as PollTier },
    { id: 'rating', name: 'Rating Scale', icon: SlidersHorizontal, shortDesc: 'Rate each 1-5 stars', tooltip: 'Voters rate every option from 1-5 stars independently. See average ratings to compare. Great for feedback and evaluations.', gradient: 'from-cyan-500 to-blue-500', bgGlow: 'bg-cyan-500/20', tier: 'free' as PollTier },
    { id: 'rsvp', name: 'RSVP', icon: Users, shortDesc: 'Event attendance', tooltip: 'Simple Yes/No/Maybe responses for events. See who\'s coming with automatic headcounts. Perfect for party planning!', gradient: 'from-sky-500 to-blue-500', bgGlow: 'bg-sky-500/20', tier: 'free' as PollTier },
    { id: 'image', name: 'Visual Poll', icon: ImageIcon, shortDesc: 'Vote on images', tooltip: 'Upload images and let people vote visually. Perfect for design feedback, logo selection, photo contests, or product comparisons.', gradient: 'from-pink-500 to-rose-500', bgGlow: 'bg-pink-500/20', tier: 'pro_event' as PollTier }
];
        ...
        <div className="mb-10">
            <div className="grid grid-cols-3 gap-4">
                {steps.map((step, i) => (
                    ...
                    >
                        {i < 2 && (
                            <div className="hidden md:block absolute top-8 left-full w-full h-0.5 bg-gradient-to-r from-slate-200 to-transparent -z-10" />
                        )}
            ...
            <div className="max-w-6xl mx-auto px-4 py-8 md:py-12">
                ...
                <div className="grid lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {/* Poll Type Selector - More Colorful */}
                        <motion.div 
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            className="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-6 overflow-hidden relative"
                        >
                            {/* Background glow */}
                                    ...
                                    return (
                                        ...
                                        >
                                            <button
                                                ...
                                                className={`relative w-full p-3 rounded-xl border-2 transition-all text-left overflow-hidden ${
                                                    isSelected 
                                                        ? 'border-transparent shadow-lg ring-2 ring-indigo-500' 
                                            ...
                                            >
                                                ...
                                                {tierConfig.label && (
                                                    <span className={`absolute -top-1 -right-1 px-1.5 py-0.5 text-[10px] font-bold rounded-full shadow ${tierConfig.colors}`}>
                                                        {tierConfig.label}
                                                    </span>
                                                )}
                                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-2 bg-gradient-to-br ${type.gradient} shadow-md`}>
                            ...
                            {isPaidType && (
                                <motion.div 
                                    ...
                                    className="mt-4 p-4 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl flex items-start gap-3"
                                >
                                    ...
                                    <div>
                                        ...
                                        <p className="text-purple-700 text-sm mt-1">Includes 2,000 responses, 60 days active, PDF export & more. <a href="/pricing" className="underline hover:text-purple-900 font-medium">See features →</a></p>
                                    </div>
                                </motion.div>
                            )}
                        </motion.div>
                        ...
                        >
                            ...
                            <div className="mb-6">
                                ...
                                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Add context or instructions..."
                                    rows={2} className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition resize-none bg-slate-50 focus:bg-white" />
                            </div>
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {hasDuplicates && (
                                        <motion.span 
                                            initial={{ scale: 0.8 }}
                                            animate={{ scale: 1 }}
                                            className="text-xs text-red-600 bg-red-50 px-3 py-1.5 rounded-full flex items-center gap-1 font-medium"
                                        >
                                            <AlertCircle size={14} /> Duplicates detected
                                        </motion.span>
                                    )}
                                </div>
                                            ...
                                            >
                                                <div className="flex-1 relative">
                                                    <input 
                                                        ...
                                                        className={`w-full px-4 py-3.5 pr-12 border-2 rounded-xl focus:ring-2 focus:ring-indigo-500 transition ${
                                                            isDuplicate ? 'border-red-400 bg-red-50' : 'border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500'
                                                        }`} 
                                                    />
                                                    <span className={`absolute right-4 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                                                        isDuplicate ? 'bg-red-200 text-red-700' : 'bg-slate-200 text-slate-500'
                                                    }`}>{index + 1}</span>
                                                </div>
                                                ...
                                                {pollType === 'image' && (
                                                    <button onClick={() => {
                                                        ...
                                                        className={`p-3.5 rounded-xl border-2 border-dashed transition ${optionImages[index] ? 'border-emerald-500 bg-emerald-50' : 'border-slate-300 hover:border-indigo-400 hover:bg-indigo-50'}`}
                                        ...
                                        <div className="p-6 space-y-4">
                                            ...
                                            <div className="p-4 bg-gradient-to-r from-slate-50 to-indigo-50 rounded-xl border border-slate-200">
                                                <div className="flex items-center justify-between mb-2">
                                                    ...
                                                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${currentTier === 'free' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                                                        Max {maxDays} days
                                                    </span>
                                                </div>
                                                <input type="datetime-local" value={deadline} onChange={(e) => setDeadline(e.target.value)}
                                                    min={new Date().toISOString().slice(0, 16)} max={getMaxDeadline()}
                                                    className="w-full px-3 py-2.5 border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm" />
                                                {currentTier === 'free' && <p className="text-xs text-amber-600 mt-2 font-medium">⚠️ Free polls: max 7 days. <a href="/pricing" className="underline">Upgrade for longer.</a></p>}
                                            </div>
                        ...
                        <motion.button 
                            ...
                            disabled={isCreating || !title.trim() || options.filter(o => o.trim()).length < 2 || hasDuplicates}
                            whileHover={{ scale: 1.01 }}
                            whileTap={{ scale: 0.99 }}
                            className="w-full py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-600 bg-size-200 animate-gradient text-white font-bold rounded-xl hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/30 text-lg"
                        >
                            {isCreating ? <><Loader2 className="animate-spin" size={22} />Creating Poll...</> : <><Sparkles size={20} />Create Poll<ArrowRight size={22} /></>}
                        </motion.button>
                    </div>
                        ...
                        >
                            <div className="bg-white rounded-2xl shadow-lg border border-slate-200/50 overflow-hidden">
                                <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-slate-50 to-indigo-50">
                                    ...
                                    <div className="flex items-center gap-1 bg-white rounded-lg p-1 shadow-sm">
                                        <button onClick={() => setPreviewDevice('desktop')} className={`p-1.5 rounded ${previewDevice === 'desktop' ? 'bg-indigo-100 shadow-sm' : ''}`}>
                                            <Monitor size={16} className={previewDevice === 'desktop' ? 'text-indigo-600' : 'text-slate-400'} />
                                        </button>
                                        <button onClick={() => setPreviewDevice('mobile')} className={`p-1.5 rounded ${previewDevice === 'mobile' ? 'bg-indigo-100 shadow-sm' : ''}`}>
                            ...
                            <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl">
                                ...
                                <ul className="text-sm text-amber-700 space-y-1.5">
                                    <li className="flex items-start gap-2"><span>•</span> Keep your question clear and specific</li>
                                    <li className="flex items-start gap-2"><span>•</span> 4-6 options works best for most polls</li>
                                    <li className="flex items-start gap-2"><span>•</span> Save your admin link to manage later</li>
                                </ul>
                            </div>
