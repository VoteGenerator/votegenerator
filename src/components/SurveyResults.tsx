...
interface DateFilterProps {
    responses: SurveyResponse[];
    onFilter: (filtered: SurveyResponse[]) => void;
...
const DateFilter: React.FC<DateFilterProps> = ({ responses, onFilter, canUseCustom }) => {
    ...
    const applyFilter = (filterType: string) => {
        ...
        const filtered = responses.filter(r => {
            const responseDate = new Date(r.submittedAt || r.startedAt || '');
...
const SurveyResults: React.FC<SurveyResultsProps> = ({ poll, responses: rawResponses, isAdmin = false, onUpgrade }) => {
    ...
    // Filtered responses (for date filtering)
    ...
    // Use filtered responses for display
    const responses = filteredResponses.length > 0 ? filteredResponses : allResponses;
    ...
    // Handle date filter
    const handleDateFilter = useCallback((filtered: SurveyResponse[]) => {
