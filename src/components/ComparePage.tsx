// ============================================================================
// ComparePage - Why VoteGenerator vs Others + Geo Pricing
// ============================================================================
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
            ...
            <div className="max-w-6xl mx-auto px-4 pt-16 pb-12 text-center">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full text-sm font-medium mb-6"><Shield size={16} /> Why teams choose us</motion.div>
                <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">The Privacy-First Poll Tool</motion.h1>
                <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-xl text-slate-600 max-w-2xl mx-auto">No signups, no emails, no tracking. Just instant polls.</motion.p>
            </div>

            <div className="max-w-6xl mx-auto px-4 pb-16">
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {benefits.map((b, i) => (
                        <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }} className="bg-white rounded-2xl p-6 border border-slate-200 hover:shadow-lg transition">
                            <div className={`w-12 h-12 bg-gradient-to-br ${b.color} rounded-xl flex items-center justify-center mb-4`}><b.icon className="text-white" size={24} /></div>
                        ...
                        <tbody>
                            {competitors.map((c, i) => (
                                <tr key={i} className={`border-b border-slate-100 ${c.highlight ? 'bg-indigo-50' : ''}`}>
                                    <td className={`py-4 px-6 font-medium ${c.highlight ? 'text-indigo-700' : 'text-slate-900'}`}>{c.highlight && <Star className="inline mr-2 text-amber-500" size={16} />}{c.name}</td>
                                    <td className="py-4 px-4 text-center">{!c.signup ? <Check className="inline text-emerald-600" size={20} /> : <X className="inline text-slate-400" size={20} />}</td>
                ...
                <div className="max-w-6xl mx-auto px-4">
                    <div className="text-center mb-8">
                        ...
                        <div className="mt-4 flex items-center justify-center">
                            {loading ? <span className="flex items-center gap-2 text-sm text-slate-500"><Loader2 size={14} className="animate-spin" /> Loading...</span> : <span className="flex items-center gap-2 text-sm text-slate-600 px-3 py-1 bg-white rounded-full border"><Globe size={14} /> Prices in <strong>{currency}</strong></span>}
                        </div>
                    </div>
                    <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl p-6 border border-slate-200">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl p-6 border-2 border-purple-500 shadow-lg relative">
                            ...
                            <div className="my-2">{loading ? <span className="text-3xl font-bold text-slate-300">...</span> : <><span className="text-3xl font-bold text-slate-900">{prices.symbol}{prices.proEvent}</span><span className="text-slate-500 ml-1">{currency}</span></>}</div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl p-6 border border-slate-200">
                            ...
                            <div className="my-2">{loading ? <span className="text-3xl font-bold text-slate-300">...</span> : <><span className="text-3xl font-bold text-slate-900">{prices.symbol}{prices.unlimited}</span><span className="text-slate-500 ml-1">{currency}</span></>}</div>
            ...
            <div className="py-16">
                <div className="max-w-4xl mx-auto px-4 text-center">
                    ...
                    <a href="/create" className="inline-flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition shadow-lg"><Sparkles size={20} /> Create Free Poll <ArrowRight size={20} /></a>
                </div>
            </div>
